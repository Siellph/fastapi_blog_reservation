...
async def create_restaurant(session: AsyncSession, restaurant_data: RestaurantCreate) -> RestaurantRead:
...
async def get_restaurant(session: AsyncSession, restaurant_id: int) -> RestaurantRead:
...
async def get_restaurants(session: AsyncSession) -> List[RestaurantRead]:
...
async def delete_restaurant(session: AsyncSession, restaurant_id: int) -> int:
    statement = select(Restaurant).where(Restaurant.id == restaurant_id)
    result = await session.execute(statement)
    restaurant = result.scalar()
    if not restaurant:
