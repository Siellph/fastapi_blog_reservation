...
async def create_dish(session: AsyncSession, dish_data: DishCreate) -> DishRead:
...
async def get_dish(session: AsyncSession, dish_id: int) -> DishRead:
...
async def get_dishes(session: AsyncSession, category: DishCategory = None) -> List[DishRead]:
...
async def update_dish(session: AsyncSession, dish_id: int, dish_data: DishUpdate) -> DishRead:
    statement = select(Dish).where(Dish.id == dish_id)
    result = await session.execute(statement)
    dish = result.scalar()
    if not dish:
...
async def delete_dish(session: AsyncSession, dish_id: int) -> int:
    statement = select(Dish).where(Dish.id == dish_id)
    result = await session.execute(statement)
    dish = result.scalar()
    if not dish:
